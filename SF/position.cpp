...
char const PieceToChar[] = " PNBRQK  pnbrqk ";
...
char* print_pos(char* s, const Position& pos) {
  s = print_string(s,"\n +---+---+---+---+---+---+---+---+\n");
  for (Rank r = RANK_8; r >= RANK_1; --r) {
    for (File f = FILE_A; f <= FILE_H; ++f) {
      s = print_string(s," | ");
      *s++ = PieceToChar[pos.piece_on(make_square(f, r))];
    }
    s = print_string(s,"|\n +---+---+---+---+---+---+---+---+\n");
  }
  s = print_string(s,"nFen: ");
  s = pos.print_fen(s);
  s = print_string(s,"\nKey: ");
  s = print_hex64(s,pos.st->key);
  s = print_string(s,"\nCheckers: ");
  for (Bitboard b = pos.checkers(); b; ) {
    s = UCI::print_square(s,pop_lsb(&b));
    *s++ = ' ';
  }
  *s++ = '\n';
  return s;
...
//  for (PieceType i = NO_PIECE_TYPE; i < PIECE_TYPE_NB; ++i)
//    for (int j = 0; j < 16; ++j)
...
const char* Position::parse_set(const char* s, bool isChess960, Thread* th) {

  unsigned char col, row, ch;
  size_t idx;
  Square sq = SQ_A8;
  ...
  while ((ch=*s++)>' ')
  {
      if ('0'<=ch && ch<='8')  sq += Square(ch - '0'); // Advance the given number of files
      else if (ch == '/') sq -= Square(16);
      else {
        for (idx=1;idx<=14;idx++) {
            if (ch == PieceToChar[idx]) {
  ...
  while (*s ==' ') {s++;}
  (ch=*s++); sideToMove = (ch == 'w' ? WHITE : BLACK);
  ...
  while ((ch=*s++)>' ')
  {
      ...
      if (ch == 'K')
          ...
          rsq = make_square(File(ch - 'A'), relative_rank(c, RANK_1));
  ...
  while (*s ==' ') {s++;}
  s = parse_int(s,st->rule50);
  while (*s ==' ') {s++;}
  s = parse_int(s,gamePly);
...
char* Position::print_fen(char* s) const {
  int emptyCnt;
// board
  for (Rank r = RANK_8; r >= RANK_1; --r) {
      for (File f = FILE_A; f <= FILE_H; ++f) {
          ...
          if (emptyCnt)
              *s++ = '0'+emptyCnt;
          if (f <= FILE_H)
              *s++ = PieceToChar[piece_on(make_square(f, r))];
      }
      if (r > RANK_1)
          *s++ = '/';
  }
...
// castling
  *s++ = ' ';
  if (can_castle(WHITE_OO))
      *s++ = (chess960 ? ('A' + file_of(castling_rook_square(WHITE |  KING_SIDE))) : 'K');
  if (can_castle(WHITE_OOO))
      *s++ = (chess960 ? ('A' + file_of(castling_rook_square(WHITE | QUEEN_SIDE))) : 'Q');
  if (can_castle(BLACK_OO))
      *s++ = (chess960 ? ('a' + file_of(castling_rook_square(BLACK |  KING_SIDE))) : 'k');
  if (can_castle(BLACK_OOO))
      *s++ = (chess960 ? ('a' + file_of(castling_rook_square(BLACK | QUEEN_SIDE))) : 'q');
  ...
  *s++ = ' ';
  if (ep_square() == SQ_NONE) {
    *s++ = '-';
    ...
    s = UCI::print_square(s,ep_square());
  }
  *s++ = ' ';
// 50 move rule
  s = print_int(s,st->rule50);
  *s++ = ' ';
// move count
  s = print_int(s,1 + (gamePly - (sideToMove == BLACK)) / 2);
