...
; eval
	...
	;  and are added to the map
		...
		mov   dword[rbx+4*rsi], eax
	     ...
	     Assert   ne, byte[rdi+(ENDGAME_EVAL_MAP_SIZE-1)*sizeof.EndgameMapEntry+EndgameMapEntry.entry], 0, 'problem1 in Endgame_Init'
...
; scale
	...
	;  and are added to the map
		...
		mov   dword[rbx+4*rsi], eax
	     ...
	     Assert   ne, byte[rdi+(ENDGAME_SCALE_MAP_SIZE-1)*sizeof.EndgameMapEntry+EndgameMapEntry.entry], 0, 'problem2 in Endgame_Init'
...
.Next:
		...
		mov   edx, dword[rdi+EndgameMapEntry.entry]
	       test   edx, edx
		 jz   .AtEnd
...
.AtEnd:
		...
		mov   dword[rdi+EndgameMapEntry.entry], esi
		mov   rcx, rax
		mov   esi, edx
